...
"""
Standalone 3D Multi-Object Tracking Script with Rotation, ROI, and Speed Estimation
Applies same rotation and ROI filtering as bounding box generation process
Includes speed estimation and display for engineering vehicles
No ROS workspace required - runs in conda environment
"""
...
FRAME_RATE = 5.0  # Hz - frame rate for speed calculation
MIN_SPEED_THRESHOLD = 0.1  # m/s - minimum speed to display
MAX_REASONABLE_SPEED = 20.0  # m/s - maximum reasonable speed for construction vehicles
...
class SimpleKalmanFilter:
    """Simplified Kalman Filter for 3D object tracking with classification and speed estimation"""
    def __init__(self, bbox_data, track_id, dt=0.2):
        ...
        # State: [x, y, z, yaw, l, w, h, vx, vy, vz] + classification
        ...
        # Speed tracking
        self.speed_history = []  # Store recent speed values for smoothing
        self.max_speed_history = 5  # Number of speed values to keep
        self.last_position = np.array([self.state[0], self.state[1], self.state[2]])
    ...
    def update(self, measurement):
        """Update with measurement and calculate speed"""
        # Calculate speed from position change
        current_position = np.array([measurement[0], measurement[1], measurement[2]])
        current_time = time.time()
        
        if hasattr(self, 'last_position') and hasattr(self, 'last_time'):
            dt_actual = current_time - self.last_time
            if dt_actual > 0.01:  # Avoid division by very small numbers
                position_change = current_position - self.last_position
                instantaneous_speed = np.linalg.norm(position_change) / dt_actual
                
                # Filter out unreasonable speeds
                if instantaneous_speed <= MAX_REASONABLE_SPEED:
    ...
    def get_speed(self):
        """Get smoothed speed estimate in m/s"""
        if len(self.speed_history) == 0:
            return 0.0
        
        # Use median for robust speed estimation
        smoothed_speed = np.median(self.speed_history)
        return smoothed_speed if smoothed_speed >= MIN_SPEED_THRESHOLD else 0.0
    
    def get_velocity_vector(self):
        """Get velocity components from Kalman filter state"""
        return self.state[7:10]  # [vx, vy, vz]
    
    def get_kalman_speed(self):
        """Get speed from Kalman filter velocity estimate"""
        velocity = self.get_velocity_vector()
        speed = np.linalg.norm(velocity[:2])  # Use only horizontal velocity
        return speed if speed >= MIN_SPEED_THRESHOLD else 0.0

class Simple3DTracker:
    """Simplified 3D Multi-Object Tracker with speed estimation"""
    ...
    def update(self, detections):
        ...
        for tracker in self.trackers:
            if tracker.time_since_update < 1 and (tracker.hit_streak >= self.min_hits or self.frame_count <= self.min_hits):
                ...
                speed = tracker.get_speed()  # Position-based speed
                kalman_speed = tracker.get_kalman_speed()  # Kalman filter speed
...
class StandaloneTrackingNode:
    """Standalone tracking node with rotation, ROI, and speed estimation"""
    def __init__(self):
        # Initialize ROS node
    ...
    def process_frame(self, event):
        ...
        for track in tracks:
            if track['classification'] == 2:  # Vehicle
                speed_ms = track['speed']
                speed_kmh = speed_ms * 3.6
                kalman_speed_ms = track['kalman_speed']
                kalman_speed_kmh = kalman_speed_ms * 3.6
                rospy.loginfo(f"Vehicle ID {track['id']}: Speed = {speed_ms:.2f} m/s ({speed_kmh:.1f} km/h), "
                            f"Kalman = {kalman_speed_ms:.2f} m/s ({kalman_speed_kmh:.1f} km/h)")
    ...
    def publish_tracks(self, tracks):
        ...
        for i, track in enumerate(tracks):
            ...
            speed = track['speed']  # m/s
            kalman_speed = track['kalman_speed']  # m/s from Kalman filter
            ...
            kalman_speed_kmh = kalman_speed * 3.6
            
            if classification == 2 and speed > MIN_SPEED_THRESHOLD:  # Vehicle with significant speed
            ...
            if classification == 2 and speed > MIN_SPEED_THRESHOLD:
                ...
                if velocity_magnitude > 0.1:  # Only show if moving
                    ...
                    # Scale arrow based on speed (length proportional to speed)
                    arrow_length = min(speed * 2.0, 10.0)  # Scale factor, max 10m
                ...
                # Speed text with both measurements
                speed_text_marker.text = f"{speed:.1f} m/s\n{speed_kmh:.1f} km/h"
...
def main():
    try:
        ...
        print("Starting standalone tracking node with rotation, ROI, and speed estimation...")
        ...
        print(f"Speed estimation: Frame rate={FRAME_RATE}Hz, Min threshold={MIN_SPEED_THRESHOLD}m/s, Max reasonable={MAX_REASONABLE_SPEED}m/s")
        print("\nTopics published:")
        print("  - /tracking_bboxes (BoundingBoxArray)")
        print("  - /tracking_markers (MarkerArray) - labels with speed info")  
        print("  - /tracking_boxes_markers (MarkerArray) - bounding boxes")
        print("  - /speed_markers (MarkerArray) - speed vectors and text")
        print("  - /pointcloud (PointCloud2) - rotated and ROI filtered")
        print("  - /roi_markers (MarkerArray) - ROI boundary visualization")
        print("\nSpeed visualization:")
        print("  - Green arrows/text: < 2.0 m/s (< 7.2 km/h)")
        print("  - Yellow arrows/text: 2.0-5.0 m/s (7.2-18 km/h)")
        print("  - Red arrows/text: > 5.0 m/s (> 18 km/h)")
        print("  - Arrow length proportional to speed")
        print("  - Speed displayed in both m/s and km/h")
