...
def update_scene():
    ...
    fg_points = []
    for pt in curr_pc.points:
        _, idx, _ = bg_kdtree.search_radius_vector_3d(pt, voxel_size)
        if len(idx) == 0:
            fg_points.append(pt)
    ...
    if len(fg_points) > 0:
        ...
        # === DBSCAN clustering
        t4 = time.time()
        labels = np.array(fg_pc.cluster_dbscan(eps=dbscan_eps, min_points=min_points))
